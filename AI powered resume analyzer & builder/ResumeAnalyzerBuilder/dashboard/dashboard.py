...
class DashboardManager:
        ...
        for period, start_date in [
            ...
            if row:
                metrics[period] = {
                    ...
                    'ats_score': 0,  # Not stored in current schema
                    'keyword_score': 0,  # Not stored in current schema
                    'high_scoring': 0  # Not stored in current schema
                }
            else:
                metrics[period] = {
                    'total': 0,
    ...
    def get_job_category_stats(self):
        ...
        for resume in resumes:
            try:
                ...
                if not target_category:
                    # Fallback to target_category field if title is not present
        ...
        # Since we don't have success rates in the current schema, we'll use counts
        success_rates = counts  # Placeholder, as we don't have ATS scores
    ...
    def render_dashboard(self):
        """Prepare data for dashboard rendering in Flask"""
        # Quick Stats
        metrics = self.get_resume_metrics()
        total_resumes = metrics['All Time']['total']
        # Since we don't have ATS scores in the current schema, we'll use placeholders
        avg_ats = 0  # Placeholder
        high_performing = 0  # Placeholder
        success_rate = 0  # Placeholder
        
        # Trend indicators (simplified since we don't have historical data comparison)
        trend_indicators = {
