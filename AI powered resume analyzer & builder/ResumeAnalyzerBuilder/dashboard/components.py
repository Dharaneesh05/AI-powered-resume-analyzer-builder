...
import plotly.io as pio  # Added for to_json
import pandas as pd
...
class DashboardComponents:
    ...
    def create_gauge_chart(self, value, title, chart_id):
        ...
        return pio.to_json(fig)  # Use Plotly's to_json instead of json.dumps

    def create_bar_chart(self, categories, counts, title, chart_id):
        """Create a bar chart for skill distribution or job categories"""
        # Create a DataFrame from categories and counts
        df = pd.DataFrame({
            'Category': categories,
            'Count': counts
        })
        
        # Use the DataFrame with px.bar
        fig = px.bar(
            df,
            x='Count',
            y='Category',
    ...
    def create_trend_chart(self, dates, values, title, chart_id):
        """Create a trend chart for weekly submissions"""
